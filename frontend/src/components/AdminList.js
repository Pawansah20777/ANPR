...
import './AdminList.css'; // Ensure you have a separate CSS file for styles
...
const AdminList = () => {
    ...
    useEffect(() => {
        const fetchPlates = async () => {
            try {
                const response = await axios.get('http://localhost:5000/detected_plates');
    ...
    const handleDelete = async (id) => {
        const confirmDelete = window.confirm("Are you sure you want to delete this plate?");
                    ...
                    <tbody>
                        {plates.map((plate, index) => (
                            <tr key={plate._id}>
                                ...
                                <td>
                                    <img src={plate.image} alt={plate.plate} style={{ width: '100px', height: 'auto' }} />
                                </td>
                                <td>
                                    <button
